...
$.getJSON("../JSONData.json", function(data) {
    ...
    $.each(data, function(key, val){
        ...
        $.each(season, function(key, value) {
            ...
            var seasonObj = this;
            output += "<li class='sub-sub-nav'>" + key ;
            output += "<br>";
            output += "<ul>";
...
/* For collapsing and uncollapsing elements */

  $('.parent').on("click",function(){

    $(this).find(".sub-nav").toggle();
    $(this).find(".sub-nav .sub-sub.nav").toggle();
    $(this).siblings().find(".sub-nav").hide();
    $(this).siblings().find(".sub-nav .sub-sub-nav").show();
